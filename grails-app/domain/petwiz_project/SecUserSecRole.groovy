...
class SecUserSecRole implements Serializable {
	...
	private static DetachedCriteria criteriaFor(long secUserId, long secRoleId) {
		SecUserSecRole.where {
			secUser == Person.load(secUserId) &&
			secRole == Rol.load(secRoleId)
		}
	}

	static SecUserSecRole create(Person secUser, Rol secRole, boolean flush = false) {
	...
	static constraints = {
		secRole validator: { Rol r, SecUserSecRole ur ->
